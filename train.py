...
def train(train_loader, model):
    ...
    for batch in train_loader:
        ...
        if pbar is not None:
            pbar.set_description('loss:{:.4f}'.format(batch_loss[0].item()))
            pbar.update(1)
...
def main(config_, save_path, args):
    ...
    if local_rank == 0:
        decoder_params, csa_blk_params, mrm_blk_params = 0, 0, 0
        for (name, param) in model.named_parameters():
            if "csa_blocks" in name:
                csa_blk_params += param.numel()
            elif "mrm_blocks" in name:
                mrm_blk_params += param.numel()
            elif "mask_decoder" in name:
                decoder_params += param.numel()                

        print('conv side adapter block params:{}'.format(csa_blk_params))
        print('multi-scale refine module params:{}'.format(mrm_blk_params))
        print('mask decoder params:{}'.format(decoder_params))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default="./configs/cod-tssam-vit-h.yaml")
